...
void ofApp::update() {
	...
	for (int z = -900; z <= 900; z += 10) {
		...
		for (int deg = 0; deg < 360 + tmp_deg; deg += 1) {
			...
			auto noise_value = ofNoise(glm::vec4(radius * cos(deg * DEG_TO_RAD) * noise_step, radius * sin(deg * DEG_TO_RAD) * noise_step, z * noise_step, ofGetFrameNum() * 0.01));
			if (noise_value < 0.45) {
				...
				if (deg == 0) {
					...
					while (true) {
						...
						tmp_deg -= 1;
						auto tmp_noise_value = ofNoise(glm::vec4(radius * cos(tmp_deg * DEG_TO_RAD) * noise_step, radius * sin(tmp_deg * DEG_TO_RAD) * noise_step, z * noise_step, ofGetFrameNum() * 0.01));
				...
				if (deg_start < deg_end) {
					...
					this->setRingToMesh(this->face, this->frame, glm::vec3(0, 0, z + 8), radius, 5, 9, deg_start, deg_end);
				}
